...
namespace Raven.Database.Server
{
    public class HttpServer : IDisposable
    {
        ...
        public void Start()
        {
            if (Configuration.ListenerProtocol == ListenerProtocol.Tcp)
            {
                StartTcp();
            }
            else
        ...
        {
            // Haven't implemented proper security in TCP stack. 
           // Since it's intended for use on an internal Azure worker role, this shouldn't be a problem.
        ...
        private void GetTcpContext(IHttpContext ctx)
        {
            if (concurretRequestSemaphore.Wait(TimeSpan.FromSeconds(5)) == false)
            {
                HandleTooBusyError(ctx);
